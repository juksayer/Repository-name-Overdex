...
class AnnouncementWitness(
    ...
    private val recognize: suspend (Bitmap?, String) -> List<RecognitionResult<*>> = { cropped, regionId ->
        ...
        else ObservationRecognizer.recognize(CaptureObservation(regionId, cropped), stage = "MATCH_START")
    },
    ...
    override fun start(match: Match) {
        ...
        newScope.launch {
            input.supply { bitmap ->
                Log.d("AnnouncementWitness", "bitmap received")
                ...
                val recognitionResults = if (cropped != null) {
                    recognize(cropped, "Announcement")
                } else if (bitmap == null) {
                    // Support for JVM unit tests where Bitmap class exists but methods are stubs
                    recognize(null, "Announcement")
                } else {
                    emptyList()
                }

                val result = recognitionResults
                    .firstOrNull { it.recognizer == "AnnouncementRecognizer" && it.value is String }

                if (result != null) {
                    Log.d(
                        "AnnouncementWitness",
                        "Announcement OCR text submitted: \"${result.value}\" confidence=${result.confidence}"
                    )
