...
object ObservationRecognizer {
    ...
    suspend fun recognize(observation: CaptureObservation, stage: String = "UNKNOWN"): List<RecognitionResult<*>> {
        ...
        return results.filter { it.recognizer == "YouWinRecognizer" || it.recognizer == "GoodEffortRecognizer" || it.recognizer == "AnnouncementRecognizer" || it.recognizer == "SpeciesNameRecognizer" || (it.confidence != null && it.confidence > 0) }
    }

    private fun recordAttempt(regionId: String, stage: String, result: RecognitionResult<*>): RecognitionResult<*>? {
        val success = ((regionId == "YouWin" || regionId == "GoodEffort" || regionId == "Announcement" || regionId == "SpeciesName") && result.value != null) || (result.confidence != null && result.confidence > 0)
